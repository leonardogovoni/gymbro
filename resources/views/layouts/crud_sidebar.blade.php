<div x-data="{ showSidebar: false }">
	<!-- Sidebar button -->
	<button x-on:click="showSidebar = ! showSidebar" type="button" class="inline-flex items-center p-2 my-2 ms-3 text-sm text-gray-500 rounded-lg sm:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600 ">
	...
	<aside x-bind:class="showSidebar ? 'translate-x-0' : '-translate-x-64'" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 border-x">
		<div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
			...
			<ul class="font-medium pb-2 border-b">
				<li class="flex items-center p-2 text-gray-900 dark:text-white">
					<x-application-logo class="h-6 w-auto fill-gray-800 dark:fill-gray-200" />
					<span class="ml-3">{{ config('app.name', 'Laravel') }}</span>
				</li>			
			</ul>

			<ul class="pt-2 space-y-2 font-medium">
				<li>
	...
	<div class="fixed z-30 inset-0 bg-gray-900 bg-opacity-40" x-bind:class="showSidebar ? '' : 'hidden'" x-on:click="showSidebar = false">
	</div>
</div>