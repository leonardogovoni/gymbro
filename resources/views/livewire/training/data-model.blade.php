<form class="p-4 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100" wire:submit.prevent="submit">
	@if($this->exercises()->isNotEmpty())
		<div class="exercise-item p-4 bg-white dark:bg-gray-900 rounded-lg shadow text-center">
			<h3 class="text-lg font-bold mb-2 text-gray-900 dark:text-white">
				{{ $this->exercises()[$index_current]->name }}: {{ $this->exercises()[$index_current]->pivot->series }} x
				{{ $this->exercises()[$index_current]->pivot->repetitions }}
			</h3>

			<div class="flex items-center justify-center w-full h-full">
				<img src="{{ asset('images/exercises/'.$this->exercises()[$index_current]->image) }}"
					alt="{{ $this->exercises()[$index_current]->name }}"
	...
	<div class="p-4 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100">
		...
		<table class="w-full border-collapse border border-gray-300 dark:border-gray-600 rounded">
			...
			<tbody>
				@foreach(range(1, $this->exercises()[$index_current]->pivot->series) as $serie)
					<tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
						...
						<td class="p-2 border border-gray-300 dark:border-gray-600">
							@unless (strpos($this->exercises()[$index_current]->pivot->repetitions, 'MAX') !== false)
								<input type="number" class="w-full p-1 rounded border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white" placeholder="{{ $this->exercises()[$index_current]->pivot->repetitions }}" />
							@else
								<input type="number" class="w-full p-1 rounded border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white" placeholder="{{ $this->exercises()[$index_current]->pivot->repetitions }}" />
							@endunless
						</td>
