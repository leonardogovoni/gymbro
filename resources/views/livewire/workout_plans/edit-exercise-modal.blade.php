<form wire:submit="save">
	<div class="flex justify-between items-center pb-4">
		...
		<x-text-input class="w-1/3" type="number" min="0" step="1" required wire:model.live="rest" />
	</div>
	...
	@if(!$maximal)
		...
		@if($same_reps)
			<div class="flex justify-between items-center pb-4">
				...
				<x-text-input class="w-1/3" type="number" min="1" step="1" required wire:model.live="reps.0" />
			</div>
		@else
			@foreach(range(1, $sets) as $set)
				<div class="flex justify-between items-center pb-4">
					<p class="">Numero di ripetizioni (Serie {{ $set }})</p>
					<x-text-input class="w-1/3" type="number" min="1" step="1" required wire:model.live="reps.{{ $set-1 }}" />
				</div>
			@endforeach
		@endif
	@endif
